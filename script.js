        ...
        moves.forEach(move=> {
            if (
                ...
                currentPosition[1] + move[1] <= 7
                    ...
                    const newposition = [currentPosition[0]+move[0], currentPosition[1]+move[1]] //LEGAL moves
                    //Make sure new position does not exist in path
                    const notInPath = (previousPosition) =>  !(previousPosition[0] == newposition[0] && previousPosition[1] == newposition[1])
...
const knightMoves = function(currentknight, target = [3,7], queue = []) {
    if (currentknight.position[0] == target[0] && currentknight.position[1] == target[1]) {
        console.log('ye');
...
const Ui = function () {
    ...
    const userAction = function(e){
        ...
        const knightPath = knightMoves(knight(knightPosition), [targetX, targetY]).path
